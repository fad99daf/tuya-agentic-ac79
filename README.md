...
- (复位前会停 BLE 广播、主动断开活动连接并退出 BLE 模块，避免软复位后控制器状态不干净导致 App 扫不到设备，详见 FAQ)
...
| **长按 K6 或失败重配后 App 扫不到设备**(按 reset 键却成功) | 软复位(P33)不像掉电那样彻底重置 BT 控制器。固件会在复位前停广播、主动断链、等待 HCI 完成并退出 BLE 模块；真机准出前可用断电冷启动临时恢复 |
