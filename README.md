...
- 按键 **K1–K8**(PB1 上的 AD 阶梯);其中 **K6 = KEY_PHOTO,长按 = 尽力通知云端后恢复出厂**
- 串口(烧录 + 日志,默认 115200)
...
- **长按 K6 键** → 若网络和 IoT client 可用，发送一次云端 `tuya.device.reset` 请求但不读取响应 → 停止 MQTT、清除 VM 三元组并软复位 → 重启自动重新配网
- 网络不可用或 client 尚未建立时，跳过云端通知并直接恢复出厂；因此 K6 不依赖联网。此场景云端/App 可能保留旧设备，用户可在 App 中移除。
...
| **长按 K6 后没有复位** | K6 不依赖云端响应；检查串口中的 `K6 local reset marker write failed` 或供电/按键日志。离线重置后 App 仍可能有旧设备，需在 App 中移除。 |
...
> `AI-AGENT-API.md` 是涂鸦官方 `thing.ai.*` 设备侧接口规格(智能体配置/角色/聊天/音色),供需要对应云端能力的开发者参考;`FACTORY-RESET.md` 对应的 K6 尽力云端通知、始终本地恢复出厂已合入本仓代码；`WEATHER.md` / `TIME-SYNC.md` 仍是参考实现说明，需要时可按文档落地。
