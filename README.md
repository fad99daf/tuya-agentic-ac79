...
- **K6 长按云端优先恢复出厂** — 云端确认移除设备后才清除凭据并重新进入配网
...
- 按键 **K1–K8**(PB1 上的 AD 阶梯);其中 **K6 = KEY_PHOTO,长按 = 云端解绑确认后恢复出厂**
- 串口(烧录 + 日志,默认 115200)
...
- **长按 K6 键** → 请求云端 `tuya.device.reset` → 收到明确成功 → 停止 MQTT、清除 VM 三元组并软复位 → 重启自动重新配网
- 网络不可用、TLS/时间异常或云端拒绝时，设备保留本地凭据且不复位；恢复后再次长按 K6 重试，避免 App 残留旧设备。
...
| **长按 K6 后没有复位** | K6 需先完成云端解绑。检查网络、设备时间和串口中的 `K6 cloud device removal failed`；失败时凭据会保留，修复条件后再次长按即可 |
...
> `AI-AGENT-API.md` 是涂鸦官方 `thing.ai.*` 设备侧接口规格(智能体配置/角色/聊天/音色),供需要对应云端能力的开发者参考;`FACTORY-RESET.md` 对应的 K6 云端优先恢复出厂已合入本仓代码；`WEATHER.md` / `TIME-SYNC.md` 仍是参考实现说明，需要时可按文档落地。
