...
static int local_music_dec_file(void *file, int breakpoint, void *handler, int arg)
{
    ...
    void *emitter = get_bt_emitter_audio_server();
    if (!emitter) {
        if (bt_emitter_role_get() == BT_EMITTER_EN && get_total_connect_dev() > 0 && (get_emitter_curr_channel_state() & A2DP_SRC_CH)) {
            bt_emitter_pp(1);
            os_time_dly(50);
        }
    }
...
__again:
    ...
    if (strncmp(path, CONFIG_MUSIC_PATH_FLASH, strlen(CONFIG_MUSIC_PATH_FLASH)) == 0) {
        strcpy(path, CONFIG_MUSIC_PATH_FLASH);
    }
#endif
...
#if defined LOAD_SDRAM_PROMPT_PLAY && (LOAD_SDRAM_PROMPT_PLAY == 0)
    const char *path = CONFIG_VOICE_PROMPT_FILE_PATH;
#if defined CONFIG_VOICE_PROMPT_FILE_SAVE_IN_RESERVED_EXPAND_ZONE || defined CONFIG_VOICE_PROMPT_FILE_SAVE_IN_RESERVED_ZONE
    path = CONFIG_MUSIC_PATH_FLASH;
#endif

    if (!__this->local_path || (__this->local_path && strcmp(__this->local_path, CONFIG_MUSIC_PATH_FLASH) != 0)) {
        if (__this->fscan) {
            fscan_release(__this->fscan);
            __this->fscan = NULL;
            fscan_exit = 1;
...
#elif defined LOAD_SDRAM_PROMPT_PLAY && (LOAD_SDRAM_PROMPT_PLAY == 1)
    printf("file name : %s\n\r", fname);
...
static int aux_music_dec_file(void *priv, int breakpoint, void *handler, int arg)
{
    ...
    req.enc.channel_bit_map = CONFIG_AUDIO_LINEIN_CHANNEL_MAP;
#endif
...
#if BT_SUPPORT_RECEIVER_AUTO_A2DP_START
#ifdef CONFIG_BT_ENABLE
        if (__this->bt_connecting && !__this->bt_emitter_enable) {
            if (get_a2dp_drop_frame_timer() == 0 && get_total_connect_dev() == 1 && get_bt_music_dec_ops()->dec_status(0) != AUDIO_DEC_START) {
...
#ifdef CONFIG_UI_ENABLE
    if (__this->mode >= LOCAL_MUSIC_MODE && __this->mode <= UDISK1_MUSIC_MODE) {
        os_taskq_post(POST_TASK_NAME, 1, UI_MSG_PAUSE_START);
    } else {
        os_taskq_post(POST_TASK_NAME, 1, UI_MSG_PAUSE_STOP);
    }
...
static int app_music_key_click(struct key_event *key)
{
    ...
    case KEY_UP:
        if (0 == __set_dec_volume(-VOLUME_STEP) && 0 == __this->dec_ops->dec_volume(-VOLUME_STEP)) {
            ...
            syscfg_write(CFG_MUSIC_VOL, &__this->volume, sizeof(__this->volume));
#endif
#if TCFG_USER_EMITTER_ENABLE && BT_SUPPORT_MUSIC_VOL_SYNC
#if CONFIG_POWER_ON_ENABLE_EMITTER
    ...
    case KEY_ENC:
        ...
        if (get_total_connect_dev() > 0) {
...
#endif
            {
                ...
                } else if (get_call_status() == BT_CALL_ACTIVE || get_call_status() == BT_CALL_OUTGOING) {
...
static int app_music_device_event_handler(struct sys_event *sys_eve)
{
    ...
    if (sys_eve->from == DEVICE_EVENT_FROM_SD) {
        ...
        switch (device_eve->event) {
        case DEVICE_EVENT_IN:
            extern int uvc_video_jpeg2yuv_start(int usb_id);
            uvc_video_jpeg2yuv_start(((const char *)device_eve->value)[3] - '0');
            break;
        case DEVICE_EVENT_OUT:
            extern int uvc_video_jpeg2yuv_stop(void);
            uvc_video_jpeg2yuv_stop();
            break;
        }
#endif
    }
...
#elif BT_SUPPORT_WIFI_CFG_COEXISTENCE
...
#ifdef CONFIG_BT_ENABLE
#if TCFG_BD_NUM == 2
            if (!strcmp((const char *)it->data, "BtDisc.mp3") && get_total_connect_dev() > 0) {
