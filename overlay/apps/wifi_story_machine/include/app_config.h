 ...
 *          唤醒词开 15s 唤醒窗,窗内 VAD+能量门照常起轮;窗过期回睡眠。
 *          v2 包 create(NULL) 默认模型 fsmn_v8_0515_avg,官方 token:
 *          你好涂鸦={23,4,27,9,22,5,38,1}(主)、嘿涂鸦={27,8,22,5,38,1},
 *          阈值均 0.7,详见 kws/tuya_kws.c v7 注释与 docs/WAKEWORD.md。
 * 注释  = 关闭【当前默认】: 不调引擎、不喂帧,回到常听模式(行为与未接入
 *          唤醒词完全一致;库仍在链接行,无引用 LTO 裁掉不占空间)。
 *          ⚠️ 开关是 #ifdef 语义(demo.c 全部按"是否定义"编译),置 0 无效,
 *          要开就把下面整行的注释去掉;要关必须整行注释掉。 */
/* #define TUYA_KWS_ENABLE               1 */
...
// #define CONFIG_AEC_LINEIN_CHANNEL_ENABLE   //AEC硬件回采(通道3采DAC)。实测未生效——自说自话照旧,通道3大概率没接DAC环采(喂垃圾参考)。已回退走软件参考(output_way=0)。要重试需先确认板子7916AB的ch3环采通路
